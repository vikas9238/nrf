...
$sub_title = "";
if(isset($_GET['c'])){
    $cat_qry = $conn->query("SELECT * FROM product where md5(id) = '{$_GET['c']}'");
...
if(isset($_GET['s'])){
    $sub_cat_qry = $conn->query("SELECT * FROM company_list where md5(id) = '{$_GET['s']}'");
            ...
            <div class="<?php echo (isset($_GET['c']))? 'col-md-9': 'col-md-12' ?>">
                ...
                <div class="row gx-2 gx-lg-2 row-cols-1 row-cols-sm-1 row-cols-md-3 row-cols-xl-4">
                    ...
                    <?php 
                        ...
                        $sql = "SELECT b.*,c.category, d.name from `quotation_list` b inner join product c on b.product_id = c.id inner join company_list d on b.company_id = d.id  where b.status = 1 {$whereData} order by rand()";
